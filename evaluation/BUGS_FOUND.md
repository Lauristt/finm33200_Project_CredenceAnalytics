...
## Bug 1（已修;注意:只是真实失败的约 10%,不是"根本原因")：`sec_company_facts` 没把"声明对应期间"的数字捞进证据

> ⚠️ 这个 bug 在**自造题(全年度指标)**上是主因(19/20),但在**真实新闻**上只占 ~10% 的失败。
> **没有单一根本原因**——真正的大头是"根本没拿到数据(44%)"和"细分概念缺失(22%)",见下方"失败归因"。
...
| C2. **没路由到 FRED**(宏观/商品/指数/FX/利率) | 13(26%) | ❌ | CPI、布伦特、标普500、EUR/USD → FRED 里都有,只是关键词没映射过去 |
...
- **C2(13 条非股票)**:宏观/商品/指数/FX/利率**没被路由到 FRED**;但这些(CPI/非农/WTI/布伦特/标普/EUR-USD)**FRED 里其实都有**,只是关键词太窄没映射过去 → **扩 FRED 关键词 / 按实体路由即可**(不是缺源、不是缺 key)。
