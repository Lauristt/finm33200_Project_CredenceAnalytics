...
def _agent_instructions(tool_profile: str) -> str:
    ...
    return (
        ...
        "For claims that contain relative time language ('last week', 'past month', 'past 30 days', 'YTD', 'this year'), "
        "always call resolve_time_window(claim_text, memo_date) FIRST to obtain concrete start_date and end_date values, "
        "then pass those dates to fetch_index_performance, fetch_macro_indicator, fetch_fred_series, "
        "get_historical_prices, or compare_stock_performance. "
        "For index performance claims ('S&P 500 rose X% last week', 'Nasdaq fell Y% this month'), use "
        "resolve_time_window → fetch_index_performance(index_symbol, start_date, end_date). "
        "For macro indicator claims ('CPI rose X% last month', 'WTI declined Y% past month'), use "
        "resolve_time_window → fetch_macro_indicator(indicator_name, start_date, end_date). "
